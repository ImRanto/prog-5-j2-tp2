...
public class ControleurLocation {
    ...
    public Location effectuerLocation(String identifiantObjet, String identifiantLocataire, Integer dureeJours) {
        try {
            Location location = gestionnaireLocation.louerObjet(identifiantObjet, identifiantLocataire, dureeJours);
            LOGGER.info("Location effectuée avec succès");
            return location;
        } catch (ObjetIndisponibleException | LocataireNonEligibleException | ObjetNonTrouveException e) {
